	...
	return new Promise(async (resolve, reject) => {
		let {
			...
			'headers': {
				...
				'cookie': '2297035587%3AaVdfXmsMptVlNx%3A27%3AAYeCghMev3psAQpNSfKU73HY60ZgdztUBi7j4LZO0w'
			}
		})
		let user = data
		console.log(user)
	...
	return new Promise((resolve, reject) => {
		...
		axios.get('https://www.primbon.com/arti_nama.php?nama1=' + query + '&proses=+Submit%21+')
			.then(({
				...
				let result4 = result2.split('ARTI NAMA')[1]
				let result5 = result4.split('.\n\n')
				let result6 = result5[0] + '\n\n' + result5[1]
				let result7 = result6.replace("(adsbygoogle = window.adsbygoogle || []).push({});\n\n\n"," ")
				let result8 = result7.replace("\n\n\n\n\n      ","")
				let hasil = result8.replace(", memi", "\nMemi")
				resolve({
					status: 200,
					hasil
				})
